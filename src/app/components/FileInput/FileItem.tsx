...
const fileItem = tv({
  slots: {
    container:
      'flex flex-1 items-start gap-4 rounded-lg border border-zinc-200 p-4',
    icon: 'flex h-8 w-8 items-center justify-center rounded-full border-4 border-violet-50 bg-violet-100 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500',
    deleteButton: '',
  },
  variants: {
    state: {
...
export function FileItem({ name, size, state }: FileItemProps) {
  ...
  const progressValue = state === 'complete' ? '100%' : '80%'
      ...
      {state === 'error' ? (
        ...
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <div className="flex w-full items-center gap-3">
            <div className="flex h-2 flex-1 rounded-full bg-violet-100 dark:bg-zinc-600">
              <div
