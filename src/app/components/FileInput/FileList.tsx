'use client'

import { formatBytes } from '@/utils/formatBytes'
import { useFileInput } from './Root'
import { Trash2, UploadCloud } from 'lucide-react'

export function FileList() {
  const { files } = useFileInput()
  return (
    <div className="mt-4 space-y-3">
      {files.map((file: File) => (
        <div
          key={file.name}
          className="flex flex-1 items-start gap-4 rounded-lg border border-zinc-200 p-4"
        >
          <div className="flex h-8 w-8 items-center justify-center rounded-full border-4 border-violet-50 bg-violet-100">
            <UploadCloud className="h-4 w-4 text-violet-600" />
          </div>

          <div className="flex flex-1 flex-col items-start gap-1">
            <div className="flex flex-col ">
              <span className="text-sm font-medium text-zinc-700">
                {file.name}
              </span>
              <span className="text-sm text-zinc-500">
                {formatBytes(file.size)}
              </span>
            </div>

            <div className="flex w-full items-center gap-3">
              <div className="flex h-2 flex-1 rounded-full bg-violet-100">
                <div className=" h-2 w-4/5 rounded-full bg-violet-600"></div>
              </div>

              <span className="text-sm font-medium text-zinc-700">80%</span>
            </div>
          </div>

          <button className="ml-auto rounded-md p-2 text-zinc-600 transition-all hover:bg-zinc-100">
            <Trash2 className="h-5 w-5 " />
          </button>
        </div>
      ))}
    </div>
  )
}
